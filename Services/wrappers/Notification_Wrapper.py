...
load_dotenv(dotenv_path=os.path.join(os.path.dirname(__file__), "notification.env"))
...
def send_sms(to: str, body: str) -> str:
    """Send a single SMS via Twilio. Returns the Twilio message status string."""
...
def _send_batch(recipients: list[dict], body: str, recipient_type: str | None = None) -> list[dict]:
    """
    Send SMS to a list of dicts each containing a 'phone' key.
    Returns a list of delivery status objects.
    recipient_type, if provided, is included in each status entry.
    """
    results = []
    for r in recipients:
        phone = r.get("phone", "")
        ...
        if not is_valid_e164(phone):
            log.warning("Invalid E.164 number skipped (%s): %s", recipient_type or "recipient", phone)
...
# Called by the trail-condition / hazard composite service to mass-notify
# hikers currently on an affected trail.
